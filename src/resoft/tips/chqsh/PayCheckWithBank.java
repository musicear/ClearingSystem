...
public class PayCheckWithBank implements Action{

	private static final Log logger = LogFactory.getLog(PayCheckWithBank.class);
	/**
...
//					System.out.println(""+params.get("vch_no") + "  " + params.get("addword"));
		...
		for(int i=0;i<tl_td.length;i++)
		{
			if(i == tl_td.length)
			{
				...
				params.put("tipsworkdate", tipsworkdate);
				params.put("vch_no", "" + id);
				params.put("addword", "本银行会计日最后一笔交易");
...
//					System.out.println(""+params.get("vch_no") + "  " + params.get("addword"));
	...
	{
		PayCheckWithBank test = new PayCheckWithBank();
		Message msg = null;
		try {
			test.execute(msg);

...
public class ProcessCheckAcctWithBank implements Action{
	...
	//对公明细
	public void process_cdd(String bankDate){
		...
		do{
			...
			sql+="  where substr(CDDVCHN,1,4) in ('8801','8802','8803','8804') and cddac_dat='"+bankDate+"' and CDDABS like '%"+payeeBankNo+"%'";
			...
			for (int j=0;j<queryCount;j++){
				...
				//yangyuanxu add
				info.put("payeeBankNo",payeeBankNo);
	...
	public void process_crdtl(String bankDate){
		...
		do{
			//分页查询
			String sqlwhere=" and CRDTLACTNO = SDLACTNO and CRDTLJRNNO = SDLJRNNO and CRDTLTRNDAT = SDLTRNDAT and SDLPAT like '%";
			String splitPage=" c.row_Num between "+(StartIndex)+" and "+(StartIndex+Count-1)+" ";		
			sql="select "+sqlField+" from (select b.*,rownum row_num from (";
			sql+="  select "+sqlField+" from CRDTL,SDL ";
			sql+=" where CRDTLECIND='0' and substr(CRDTLVCHNO,1,4) in ('8801','8802','8803','8804')  and CRDTLTRNDAT='"+bankDate+"' "+sqlwhere+payeeBankNo+"%'";
	...
	public void process_sdl(String bankDate){
		...
		do{
			...
			sql+="  where SDLECIND='0' and substr(SDLVCHNO,1,4) in ('8801','8802','8803','8804') and SDLTRNDAT='"+bankDate+"' and SDLPAT like '%"+payeeBankNo+"%'";
	...
	public void insertAdjustAcct(Map temp) throws Exception{		
		...
		param.put("payeeBankNo", (String)temp.get("payeeBankNo"));
	...
	public static void main(String[] args){
		...
		ProcessCheckAcctWithBank test=new ProcessCheckAcctWithBank();

...
public class SelectChargeInfoPack{
	...
	public String MakeUpCheckPackager(String  RET_STATUS) {
		...
		for(int arraycount=0;arraycount<body_data.length;arraycount++)
		{
			Element struct=array.addElement("struct");  
			String[] bodyArray=body_data[arraycount]; 
				for(int i=0;i<bodyArray.length;i++){   
					Element arraydata=struct.addElement("data").addAttribute("name", bodyArrayname[i]);
						 
					Element arrayfield=arraydata.addElement("field").addAttribute("type", bodydataType[i]).addAttribute("length", bodydateLen[i]);
						 
							if(bodyArray[i]!=null)
								arrayfield.setText(bodyArray[i]);
							else
							{ 
								arrayfield.setText("");  		
							}

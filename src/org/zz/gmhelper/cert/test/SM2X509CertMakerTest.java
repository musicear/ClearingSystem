...
public class SM2X509CertMakerTest {
    ...
    public void testMakeCertificate() {
        try {
            ...
            savePriKey("D:\\workspace\\Tips_qhd\\target\\test.sm2.pri", (BCECPrivateKey) subKP.getPrivate(),
                (BCECPublicKey) subKP.getPublic());
            SM2X509CertMaker certMaker = buildCertMaker();
            X509Certificate cert = certMaker.makeSSLEndEntityCert(csr);
            FileUtil.writeFile("D:\\workspace\\Tips_qhd\\target\\test.sm2.cer", cert.getEncoded());
        } catch (Exception ex) {
            ex.printStackTrace();
            Assert.fail();
        }
